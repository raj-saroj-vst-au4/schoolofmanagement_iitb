...
const entries = flatten("publications");

const nowYear = new Date().getFullYear();
const recentCount = entries.filter((e) => e.year && e.year >= nowYear - 5).length;
...
export const metadata: Metadata = {
  title: "Publications — SJMSOM, IIT Bombay",
  description: `${entries.length}+ publications across ${uniqueFaculty} core faculty of SJMSOM.`,
};

export default function PublicationsPage() {
  const stats = [
    { value: String(entries.length), label: "Publications" },
    { value: String(uniqueFaculty), label: "Faculty contributing" },
    { value: String(recentCount), label: "In the last 5 years" },
  ];
  ...
  return (
    <Box as="main" bg="brand.ink">
      ...
      <Box
        ...
        pb={{ base: 14, md: 20 }}
      >
        <Box position="absolute" inset={0} bgGradient="radial(circle at 30% 50%, rgba(30,95,255,0.15) 0%, transparent 60%)" />
        <Container maxW="7xl" position="relative" zIndex={2}>
          <VStack align="flex-start" spacing={6} maxW="4xl">
            ...
            <Text fontSize={{ base: "lg", md: "xl" }} color="brand.mist" maxW="2xl">
              Peer-reviewed journal articles, conference papers and book chapters from our
              core faculty
              {earliestYear ? ` — from ${earliestYear} to ${nowYear}` : ""}.
            </Text>
      ...
      <PublicationsList entries={entries} />
      <FooterCTA />
    </Box>
  );
}
