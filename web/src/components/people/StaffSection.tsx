...
type Props = {
  ...
  staff: StaffMember[];
  accent: string;
  imageDir?: string; // defaults to /media/staff/
  bg?: "ink" | "obsidian";
  emptyMessage?: string;
};
...
export function StaffSection({
  ...
  return (
    <Box
      ...
      id={id}
      py={{ base: 18, md: 24 }}
      bg={bg === "obsidian" ? "brand.obsidian" : "brand.ink"}
      borderY={bg === "obsidian" ? "1px solid rgba(255,255,255,0.06)" : undefined}
    >
      <Container maxW="7xl">
        <VStack align="flex-start" spacing={4} mb={{ base: 10, md: 14 }}>
          ...
          {blurb && (
            <Text color="brand.chalk" fontSize={{ base: "md", md: "lg" }} maxW="3xl" lineHeight={1.6}>
              {blurb}
            </Text>
          )}
        </VStack>
        ...
        {staff.length === 0 ? (
          <Box
            ...
            textAlign="center"
          >
            <Text fontSize="lg" color="brand.chalk" mb={2}>
              {emptyMessage ?? "Details coming soon."}
            </Text>
            <Text fontSize="sm" color="brand.mist">
              We're finalising the listing. Please check back.
            </Text>
          </Box>
        ) : (
